...
import { BOARD_WIDTH, BOARD_HEIGHT, TETROMINO_IDS, CELL_EMPTY } from './constants.js';
import { TETROMINOS } from './tetriminos.js';

function createEmptyBoard() {
  return Array.from({ length: BOARD_HEIGHT }, () => Array.from({ length: BOARD_WIDTH }, () => CELL_EMPTY));
}

export const DEMO_BOARD = (() => {
  const b = createEmptyBoard();
  const { I, O, T, S, Z, J, L } = TETROMINO_IDS;

  // Build a small stack at the bottom with various shapes/colors
  // Bottom row
  b[19] = [J, J, J, J, S, S, Z, Z, L, L];
  // Row 18 with a gap
  b[18] = [J, CELL_EMPTY, T, T, T, CELL_EMPTY, Z, CELL_EMPTY, L, CELL_EMPTY];
  // Row 17 scattered
  b[17][0] = I;
  b[17][1] = I;
  b[17][5] = O;
  ...
  b[10] = [I, I, T, T, CELL_EMPTY, S, S, Z, Z, J];
